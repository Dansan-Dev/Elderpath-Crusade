...
14.1) Integrated movement + attack (current gameplay model)
- Source: A Plot containing an allied MonsterGamePiece (and with at least 1 action remaining).
- Valid targets while selecting:
  - Move targets: Empty plots reachable within the piece’s Speed (BFS, 4‑directional; cannot pass through or end on occupied plots). Visual: white centered dot.
  - Attack targets: Adjacent hostile MonsterGamePieces (4‑directional only, no diagonals). Visual: red glow border on the enemy’s plot (no dot).
- Resolution rules (spends 1 action):
  - Clicking a move target moves the piece to that plot and updates GamePieceData.POSITION.
  - Clicking an attack target deals damage equal to the attacker’s damage to the defender; if defender’s health ≤ 0, it is removed. The attacker stays in place.
- Validation layers:
  - Plot implements TargetFilter to allow only (a) reachable empty plots or (b) adjacent hostile plots.
  - Board.handlePlotMove performs the final check and applies move/attack accordingly.
- Visual details:
  - White dot = potential move; red border glow = potential attack; white emerging border = currently selected/confirmed target highlight (general multi‑selection feedback).
- Actions model (prototype):
  - Remaining actions are stored in GamePiece.data under GamePieceData.ACTIONS_REMAINING; if absent, default to stats.getActions().
  - Interactions cannot start if remaining actions ≤ 0. Each move or attack consumes 1 action.
  - Turn reset is out of scope for now; future work can reset remaining actions at turn boundaries.
