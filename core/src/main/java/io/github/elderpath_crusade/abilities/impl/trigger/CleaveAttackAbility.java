 ...
 * After the primary attack damage is dealt, deals damage equal to owner's
 * effective damage
...
public class CleaveAttackAbility implements TriggeredAbility {
    ...
    public void onOwnerAttack(MonsterGamePiece owner, MonsterGamePiece target, int damage) {
        ...
        if (!(targetPosObj instanceof Board.Position targetPos))
            return;
        ...
        // Note: The PIECE_ATTACKED event is already emitted by
        // AbilityUtils.performAttack()
        // with the primary target. The cleave targets are dealt damage separately.
        // If we need to include them in the event, we would need to modify
        // BaseAttackAbility
        // or AbilityUtils to support ability-specific target collection, which is more
        // complex.
    }
}
