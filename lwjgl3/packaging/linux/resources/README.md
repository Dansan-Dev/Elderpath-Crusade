...
- ElderpathCrusade (executable launcher)
...
## Quick start
1) Extract the ZIP somewhere you control (e.g., ~/Desktop/ElderpathCrusade-linuxX64/).
...
4) Open your desktop/app menu and search for “Elderpath Crusade”.


## Installation details
- Scope: Per‑user (no sudo required, no system files modified).
- Files are copied to your home directory:
  - App directory: ~/.local/share/elderpathcrusade
  - Launcher binary: ~/.local/share/elderpathcrusade/ElderpathCrusade
  - Desktop entry: ~/.local/share/applications/elderpathcrusade.desktop
  - Icon: ~/.local/share/icons/hicolor/256x256/apps/elderpathcrusade.png
...
- You can run the game directly without installing by executing ./ElderpathCrusade from the extracted directory, but menu integration and icon will not be set up.
...
2) Terminal
   ...
   2) Run the uninstaller:
       ...
       2) Then run the uninstaller ->
         ...
         rm -rf ~/.local/share/elderpathcrusade
         rm -f ~/.local/share/applications/elderpathcrusade.desktop
         rm -f ~/.local/share/icons/hicolor/256x256/apps/elderpathcrusade.png
...
- “Permission denied” when running scripts:
  - Ensure scripts are executable: chmod +x install.sh uninstall.sh ElderpathCrusade
- Game doesn’t appear in app menu after install:
  - Run cache updates (see above) or log out and back in.
- Missing icons in the menu:
  - Ensure ~/.local/share/icons/hicolor/256x256/apps/elderpathcrusade.png exists, then run gtk-update-icon-cache -f ~/.local/share/icons/hicolor
- App won’t start when double-clicked:
  - Try running from a terminal to see messages: ~/.local/share/elderpathcrusade/ElderpathCrusade
- Assets not loading / crashes after manual moves:
  - Avoid moving individual files within ~/.local/share/elderpathcrusade. Reinstall by running install.sh again from the extracted ZIP.


## Updating the game
- Re-download the new ZIP, extract it, and run ./install.sh again. It will overwrite the previous installation under ~/.local/share/elderpathcrusade.
